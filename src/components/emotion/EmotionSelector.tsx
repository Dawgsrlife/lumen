...
const EmotionSelector: React.FC<EmotionSelectorProps> = ({ selectedMood, onMoodSelect }) => {
  return (
    ...
    >
      <h2 className="text-3xl md:text-4xl font-light text-gray-900 text-center" style={{ marginBottom: '20px' }}>
        Select Your Current Mood
      </h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 max-w-5xl mx-auto px-4">
        {(Object.keys(emotionData) as EmotionType[]).map((emotion) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`
              relative p-4 sm:p-6 lg:p-8 rounded-3xl transition-all duration-300 
          ...
          >
            <div className="text-3xl sm:text-4xl lg:text-5xl mb-3">{emotionData[emotion].emoji}</div>
            <div className="text-base sm:text-lg font-semibold">{emotionData[emotion].label}</div>
          </motion.button>
        ))}
      </div>
    </motion.div>
