...
const LumenMascot: React.FC<LumenMascotProps> = ({ currentPage }) => {
  ...
  const getGreeting = (page: string) => {
    const greetings = {
      ...
      "/about":
        "Learning about Lumen? Smart choice! I'm here to guide you through it all. 🌟",
      "/features":
        "Ooh, exploring features? I love showing off what we can do together! 💫",
      "/contact":
        "Want to meet the team? They're as awesome as you'd expect! 🌈",
      "/sign-in":
        "Welcome back! I'm so excited to see you again. Let's continue your journey! ✨",
    };
    return (
      greetings[page as keyof typeof greetings] ||
      "Hi! I'm Luna, your friendly Lumen guide! ✨"
    );
  };

  // Get appropriate messages based on current page
  const getMessagesForPage = useCallback(
    (page: string) => {
      // Welcoming messages for landing page
      if (page === "/") {
  ...
  }, [currentPage]); // Add currentPage to dependency array since we use it in updatePosition
  ...
  return (
    <div className="fixed z-[9998] pointer-events-auto">
      {/* Mascot Character */}
      <animated.div style={mascotSpring} className="relative">
        ...
        >
          ...
          <div className="relative w-20 h-20 group-hover:scale-105 transition-transform duration-300">
            ...
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-orange-200 via-orange-300 to-orange-400 shadow-lg">
              ...
              {/* Cute Fox Ears */}
              <div
                className={`absolute -top-3 left-2 w-5 h-7 transform -rotate-12 transition-transform duration-300 ${earTwitch ? "scale-110" : ""}`}
              >
              ...
              <div
                className={`absolute -top-3 right-2 w-5 h-7 transform rotate-12 transition-transform duration-300 ${earTwitch ? "scale-110" : ""}`}
              >
                <div className="w-full h-full rounded-full bg-gradient-to-br from-orange-300 to-orange-500 shadow-sm"></div>
                <div className="absolute inset-1 w-3 h-5 rounded-full bg-gradient-to-br from-purple-200 to-purple-300"></div>
              </div>
        ...
        {showGreeting && (
          ...
          >
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, rgba(255, 255, 255, 0.95) 0%, rgba(255, 140, 0, 0.1) 100%)",
              }}
            >
              ...
              {/* Speech bubble tail */}
              <div className="absolute -bottom-2 left-10 w-4 h-4 bg-white/95 transform rotate-45 border-r border-b border-orange-300" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow:
                  "0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04), 0 0 0 1px rgba(255, 255, 255, 0.8) inset",
              }}
              onMouseEnter={() => {
