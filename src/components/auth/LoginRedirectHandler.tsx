...
const LoginRedirectHandler: React.FC = () => {
  ...
  useEffect(() => {
    const checkDailyStatus = async () => {
      ...
      console.log(
        "LoginRedirectHandler: Checking daily status for user:",
        user.id
      );
      ...
      try {
        ...
        console.error(
          "LoginRedirectHandler: Error checking daily status:",
          error
        );
  ...
  if (!isSignedIn) {
    console.log(
      "LoginRedirectHandler: User not signed in, redirecting to landing"
    );
  ...
  if (hasLoggedToday) {
    console.log(
      "LoginRedirectHandler: User has already logged today, redirecting to dashboard"
    );
