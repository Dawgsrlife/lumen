...
const UnityGame: React.FC<UnityGameProps> = ({
  ...
  // Initialize Unity context with proper URLs
  const {
    unityProvider,
    isLoaded,
    loadingProgression,
  ...
  const handleGameEnd = useCallback(
    (data: UnityGameData) => {
      console.log("Unity game ended", data);
      setGameData(data);
      onGameComplete?.(data);
    },
    [onGameComplete]
  ...
  const handleAchievement = useCallback(() => {
    console.log("Unity achievement earned");
  }, []);

  // Handle reward event
  const handleReward = useCallback(
    (data: UnityReward) => {
      console.log("Unity reward earned", data);
      onRewardEarned?.(data);
    },
    [onRewardEarned]
  );

  // Handle Unity errors with more detail
  const handleError = useCallback((message: string) => {
    console.error("Unity error:", message);
    setError(`Game loading error: ${message}`);
  }, []);
  ...
  const sendUnityMessage = useCallback(
    (gameObject: string, method: string, parameter?: string) => {
      ...
      try {
        ...
        console.warn(
          `Failed to send Unity message: ${gameObject}.${method}`,
          err
        );
        return false;
      }
