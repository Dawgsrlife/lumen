...
class DatabaseService {
  ...
  async updateUser(clerkId: string, updates: Partial<UserData>): Promise<void> {
    if (!this.isConnected) {
      ...
      const userIndex = users.findIndex(
        (user: UserData) => user.clerkId === clerkId
      );
      if (userIndex !== -1) {
  ...
  async getUserEmotions(userId: ObjectId, limit = 50): Promise<EmotionEntry[]> {
    if (!this.isConnected) {
      ...
      return entries
        .filter(
          (entry: EmotionEntry) => entry.userId.toString() === userId.toString()
        )
        .sort(
          (a: EmotionEntry, b: EmotionEntry) =>
  ...
  async saveGameProgress(
    progress: Omit<GameProgress, "_id">
  ...
  async logEngagement(engagement: Omit<UserEngagement, "_id">): Promise<void> {
    if (!this.isConnected) {
      const engagements = JSON.parse(
        localStorage.getItem("userEngagement") || "[]"
      );
  ...
  async saveWeeklyInsight(
    insight: Omit<WeeklyInsight, "_id">
  ): Promise<WeeklyInsight> {
    if (!this.isConnected) {
      const insights = JSON.parse(
        localStorage.getItem("weeklyInsights") || "[]"
      );
  ...
  async getLatestWeeklyInsight(
      ...
      const userInsights = insights
        .filter(
          (insight: WeeklyInsight) =>
            insight.userId.toString() === userId.toString()
        )
        .sort(
          (a: WeeklyInsight, b: WeeklyInsight) =>
    ...
    const collection = this.getCollection<WeeklyInsight>("weeklyInsights");
    return await collection.findOne({ userId }, { sort: { generatedAt: -1 } });
  }

  // AI Feedback Storage
  async saveAIFeedback(
    userId: ObjectId,
    emotionEntryId: ObjectId,
    feedback: AIFeedback
  ): Promise<void> {
    if (!this.isConnected) {
      const aiFeedbacks = JSON.parse(
        localStorage.getItem("aiFeedbacks") || "[]"
      );
      aiFeedbacks.push({
        _id: new ObjectId(),
        userId,
